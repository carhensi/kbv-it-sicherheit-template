% PDF/A-3u + optional PDF/UA-1 Compliance + PDF/X-1a für Druck
% Drei Varianten: Standard (kompakt) vs. Accessible (mit Tagging) vs. Printable (PDF/X-1a)
% Build: make build-standard / make build-accessible / make build-printable

% Switch für Build-Varianten (wird per CLI gesetzt: \def\accessible{} oder \def\printable{})
\newif\ifaccessible
\newif\ifprintable
\ifdefined\accessible\accessibletrue\else\accessiblefalse\fi
\ifdefined\printable\printabletrue\else\printablefalse\fi

\ifprintable
  % Printable-Version: PDF/X-1a für professionellen Druck
  \documentclass[ngerman,twoside,11pt]{scrartcl}
...
\clearpage
\input{includes/993_anhang-changelog.tex}
